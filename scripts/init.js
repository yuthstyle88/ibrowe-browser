...
const syncArgs = ['run', 'sync', '--', '--init']
const forwardedArgs = process.argv.slice(2)
const requestsFullHistory = forwardedArgs.includes('--history')
const hasHistoryFlag = requestsFullHistory || forwardedArgs.includes('--no-history')

if (!hasHistoryFlag) {
  syncArgs.push('--no-history')
}
...
applyIBrowePatches()
    .then(() => {
      // Final sync step
      console.log(`Running ${npmCommand} ${syncArgs.join(' ')}`)
      const syncResult = spawnSync(npmCommand, syncArgs, {
        cwd: braveCoreDir,
        env: process.env,
        stdio: 'inherit',
        shell: true
